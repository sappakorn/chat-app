...
class SellerMessageController extends GetxController {
  final AuthController _authController = Get.put(AuthController());

  RxList<ModelSellerMessage> messagesList = <ModelSellerMessage>[].obs;
  ...
  Future<void> fetchChatData() async {
    ...
    final String sellerId = _authController.sellerId.value;
    try {
      final res = await http.get(
        Uri.parse('${Configs.apiServerUrl}/chat/by-seller/$sellerId'),
      ...
      if (res.statusCode == 200) {
        ...
        List<ModelSellerMessage> chatList =
            ModelSellerMessage.listFromJson(resJson['result']);

        messagesList.value = chatList.cast<ModelSellerMessage>();
